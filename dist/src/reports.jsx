...
function AlertsPanel({ data, onNav, onAction }) {
  ...
  const togglePromo = (i) => setPromoStates((arr) => arr.map((v, j) => j === i ? !v : v));
    ...
    <div>
      ...
      {editingOffer && (
        <OfferEditModal
          ...
          onSave={(patch) => {
            setSlowMover(editingOffer, patch);
  ...
  const handlers = {
    ...
    sales: {
      excel: () => {
        ...
        window.exportUtils.downloadCSV(`ventas-${fmtToday}.csv`, cols, rows);
      ...
      pdf: () => {
        ...
        const lineChart = window.exportUtils.lineChartSVG([
        ...
        const ticketsBar = window.exportUtils.barChartSVG(
          data.salesByDay.slice(-7).map((d) => ({ label: d.label, value: d.tickets })),
          { color: "#7b2cbf" }
        );
        window.exportUtils.printReport({
    ...
    inventory: {
      excel: () => {
        ...
        window.exportUtils.downloadCSV(`inventario-${fmtToday}.csv`, cols, rows);
    ...
    payroll: {
      excel: () => {
        ...
        window.exportUtils.downloadCSV(`nomina-quincenal-${fmtToday}.csv`, cols, rows);
      ...
      pdf: () => {
        ...
        const payrollBar = window.exportUtils.barChartSVG(
          rows.map((r) => ({ label: r.name.split(" ")[0], value: +r.total.toFixed(0) })),
          { color: "#de0fab" }
        );
        const breakdownPie = window.exportUtils.pieChartSVG([
    ...
    attendance: {
      excel: () => {
        ...
        window.exportUtils.downloadCSV(`asistencia-${fmtToday}.csv`, cols, rows);
      ...
      pdf: () => {
        ...
        const empBar = window.exportUtils.barChartSVG(
    ...
    executive: {
      excel: () => {
        ...
        window.exportUtils.downloadCSV(`resumen-ejecutivo-${fmtToday}.csv`, ["Concepto", "Valor"], rows);
      ...
      pdf: () => {
        ...
        const salesLine = window.exportUtils.lineChartSVG([
          { name: "Ventas",   color: "#de0fab", data: data.salesByDay.map((d) => ({ y: d.ventas, label: d.label })) },
          { name: "Utilidad", color: "#10b981", data: data.salesByDay.map((d) => ({ y: d.utilidad })) },
        ], { height: 200 });
        ...
        const catPie = window.exportUtils.pieChartSVG(
          data.categoryRevenue.map((c) => ({ label: c.name, value: c.value, color: c.color })),
          { height: 200 }
        );
        ...
        const attLine = window.exportUtils.lineChartSVG([
