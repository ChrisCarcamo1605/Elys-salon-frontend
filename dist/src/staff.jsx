...
function Staff({ user, data, onLock, onBack }) {
  ...
  const saveEmployee = (emp) => {
    setEmployees((es) => {
      ...
      if (idx === -1) return [...es, emp];
      const copy = [...es];
      copy[idx] = emp;
      return copy;
    });
    setEditEmp(null);
    showToast("Cambios guardados", emp.name);
  };
...
function StaffHours({ employees, historic }) {
  ...
  const exportToCSV = () => {
    ...
    window.exportUtils.downloadCSV(
...
function TimeClock({ user, data, onLock, onBack }) {
  ...
  const punchOut = () => {
    const out = nowHM();
    setEntries((es) => es.map((t) => (t.id === openEntry.id ? { ...t, out } : t)));
    setToast({ kind: "out", time: out, name: user.name, started: openEntry.in });
    setTimeout(() => setToast(null), 3000);
  };
